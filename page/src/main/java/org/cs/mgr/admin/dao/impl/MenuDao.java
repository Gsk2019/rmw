...
public class MenuDao extends BaseDao<Menu> implements IMenuDao {
	...
	public String getNewSuperMenuCode(){
		StringBuffer sb = new StringBuffer("SELECT MAX(CONVERT(CODE, UNSIGNED )) AS max_code FROM mgr_menu where pid=0");
...
//		int ret ;
		StringBuffer sb = new StringBuffer("select MAX(CONVERT(CODE, UNSIGNED )) as max_code from mgr_menu where pid = " + m.getPid());
	...
	public List<Map<String, Object>> getRoleMenu(int roleId) {
		// TODO Auto-generated method stub
		StringBuffer sb = new StringBuffer(" SELECT tmm.id AS menu_id, tmm.code as menu_code,tmm.pid as menu_pid, tmm.name AS menu_name, IFNULL((SELECT tmrm.action_val FROM mgr_role_menu tmrm LEFT JOIN  mgr_role tmr ON tmr.code = tmrm.role_code WHERE tmr.id =" + roleId + " AND tmrm.menu_code = tmm.code),-1) AS `set` FROM mgr_menu tmm  order by tmm.code, tmm.weight ");

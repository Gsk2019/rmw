            ...
            <form class="form-horizontal" action="edit.cs" id="addUserForm" method="POST" onsubmit="return false;">
                ...
                <div class="form-group col-sm-12">
                    <div class="form-group col-sm-6">
                        ...
                        <div class="col-sm-8">
                            <input id="title" name="title" class="form-control" value="${news.title}" validate="{required:true,maxlength:50}" validateMessage="{required:'请输入名称',maxlength:'输入的名称超出长度限制'}" >
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group col-sm-12">
                    ...
                    <div class="form-group col-sm-12">
                        ...
                        <div class="col-sm-10">
                            <textarea id="summary" name="summary"  placeholder="请输入摘要" class="form-control" validate="{required:true,minlength:0,maxlength:100}" validateMessage="{required:'请输入摘要',maxlength:'输入的摘要超出长度限制'}" >${news.summary}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group col-sm-12">
                    ...
                    <div class="form-group col-sm-12">
                        ...
                        <div class="col-sm-10">
                            <input id="keywords" name="keywords" value="${news.keywords}"  placeholder="请输入关键字" class="form-control" validate="{required:true,minlength:0,maxlength:100}" validateMessage="{required:'请输入关键字',maxlength:'输入的关键字超出长度限制'}" />
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-sm-8">
                            <c:forEach items="${typeList}" var="type">
                                <label class="radio-inline">
                                    <input type="radio"  value="${type.id}" <c:if test="${news.type eq type.id}">checked</c:if>  name="type" >${type.name}
                                </label>
                            </c:forEach>
                            <%--<label class="radio-inline">
                                <input type="radio"  value="1" name="type" <c:if test="${news.type eq 1}">checked</c:if> >行业动态
                            </label>
                            <label class="radio-inline">
                                <input type="radio"  value="2" name="type"  <c:if test="${news.type eq 2}">checked</c:if> >价格行情
                            </label>
                            <label class="radio-inline">
                                <input type="radio"  value="3" name="type"  <c:if test="${news.type eq 3}">checked</c:if> >独家视角
                            </label>--%>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group col-sm-12">
                    ...
                    <div class="form-group col-sm-12" <c:if test="${news.linkType==2}">style="display: none;"</c:if> >

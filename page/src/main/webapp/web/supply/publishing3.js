$(function(){
    ...
    var urlparam = getUrlParam("infoid");
    $("#info_id").val(urlparam);

    var info;

    var redata = window.getObject();
    var data1 = new Object();
    data1.infoid = urlparam;
    redata.data = JSON.stringify(data1);
    $.ajax({
        type: 'post',
        url:    window.requestUrl + "rest/supply/getInfo.cs", //即上面的接口1
        data:redata,
        async:false,
    ...
    function resultFree(data) {
        if (data.ret == 1) {
            ...
            for (var i = 0; i < obj.allcategory.length; i++) {
                if (obj.allcategory[i].topId == '0') {
                    if(info.categoryTopId == obj.allcategory[i].id){
                        $('#categoryTopId').append("<option selected value='"+obj.allcategory[i].id+"'>"+obj.allcategory[i].categoryName+"</option>")
                    }else{
            ...
            for (var i = 0; i < obj.allcategory.length; i++) {
                ...
                if(pca.id == info.categoryId){
                    $(".varieties_1").html("<select disabled name='categoryId' id='categoryId' lay-verify='required' class='varieties_1_select'></select>");
                    $('.varieties_1_select').append("<option value="+ pca.id+">"+ pca.categoryName+"</option>")
                }
            ...
            for (var i = 0; i < obj.productAreas.length; i++) {
                if (obj.productAreas[i].topId == '0') {
                    if(info.productTopAreaId == obj.productAreas[i].id){
                        $('#productTopAreaId').append("<option selected value='"+obj.productAreas[i].id+"'>"+obj.productAreas[i].areaName+"</option>")
                    }else{
            ...
            for (var i = 0; i < obj.productAreas.length; i++) {
                ...
                if(pca.id == info.productAreaId){
                    $(".place_of_origin_1").html("<select name='productAreaId' disabled id='productAreaId' lay-verify='required' class='place_of_origin_1_select'></select>");
                    $('.place_of_origin_1_select').append("<option selected value="+ pca.id+">"+ pca.areaName+"</option>")
                }
    ...
    layui.use(['layer','element','form','laydate'], function(){
        ...
        function addStock(data) {
            if (data.ret == 1) {
                layer.msg("");
                layer.confirm('发布供货信息成功，返回求供应模块？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    //此处请求后台程序，下方是成功后的前台处理……
                    window.location.href = window.requestUrl + "web/supply/supply.jsp";
                });
            }
        }
        //发布现货
        form.on('submit(release_spot)', function(data){
            ...
            $.ajax({
                type: 'post',
                url:    window.requestUrl + "rest/stock/addStocks3.cs", //即上面的接口1
                data:redata,
                success: function(data, status) {

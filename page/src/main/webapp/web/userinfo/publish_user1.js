$(function(){
    ...
    layui.use(['layer','element','form','laydate'], function(){
        ...
        function addSupply(data) {
            if (data.ret == 1) {
                layer.msg("编辑成功");
                window.location.href = window.requestUrl + "web/userinfo/my_buy.jsp";
            }
        }

        //发布求购
        form.on('submit(buy_and_buy)', function(data){
            ...
            $.ajax({
                type: 'post',
                url:    window.requestUrl + "rest/supply/editSupply.cs", //即上面的接口1
                data:redata,
                success: function(data, status) {
                    window.resultOpera(addSupply, data);
                },
                error: function(data, status) {
                    // alert(status);
                    console.log(status);

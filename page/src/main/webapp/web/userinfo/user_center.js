$(function(){
    ...
    layui.use(['layer','element','form','laydate','table'], function(){
        ...
        $('#all').click(function(){
            ...
            $('.table tbody input').prop('checked',checkbox)
        })

        //批量删除
        $('#all_del').click(function(){
            var length = $('.table tbody input:checkbox:checked').length
            var arr = ''
            if(length){
                $('.table tbody input:checkbox:checked').each(function(i){
                    arr = arr + $(this).val() + ",";
                })
        ...
        $('#all_submit').click(function(){
            ...
            if(length){
                ...
                $.ajax({
                    type: 'post',
                    url: window.requestUrl + "rest/stock/gen.cs", //即上面的接口1
                    data:redata,
        ...
        function laypagemy(obj){
            ...
            $.ajax({
                type: 'post',
                url: window.requestUrl + "rest/stock/getMyStock.cs", //即上面的接口1
                data:redata,
                success: function(data, status) {
        ...
        function resultSupply(data){
            ...
            for(var i=0;i<pager.results.length;i++){
                ...
                /*var li = '<tr>' +
                    ...
                    '</tr>';*/
                var li = '<tr>' +
                    '<td><input type="checkbox" name="" id="" value="'+pr.id+'" /></td>' +
                    '<td>'+pr.category_top_name+'-'+pr.category_name+'</td>' +
                    '<td>'+pr.brand+'</td>' +
                    '<td>'+getDateDay(pr.product_date)+'</td>' +
                    '<td class="red">'+pr.price+'元</td>' +
                    '<td>'+pr.ptop_area+'-'+pr.product_area+'</td>' +
                    '<td>'+pr.repertory+'</td>' +
                    '<td>'+pr.count+'</td>' +
                    '<td>'+getFullDayByLong(pr.create_date)+'</td>' +
                    '<td>'+showmsg+'</td>' +
                    '<td><a class="layui-btn layui-btn-xs layui-btn-normal" href="'+window.requestUrl +'web/userinfo/publishing_user1.jsp?infoid='+pr.id+'">编辑</a><a class="layui-btn layui-btn-xs layui-btn-danger" href="javascript:dels('+pr.id+');">删除</a></td>' +
                    '</tr>';
